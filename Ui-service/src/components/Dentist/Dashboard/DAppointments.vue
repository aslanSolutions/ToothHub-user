      ...
      <div class="categorized-appointments">
        ...
        <div class="done-appointments">
          ...
          <div class="appointment-card" v-for="appointment in doneAppointments" :key="appointment._id">
            <p> Date: {{ appointment.appointment_datetime }}</p>
            <p> Dentist: Dr. {{ appointment.patientName }}</p>
            <p> Status: Done</p>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async fetchAppointments() {
      try {
        const response = await axios.get(`http://127.0.0.1:5002/appointments/get_by_dentist?dentist_email=${this.dentistEmail}`);
    ...
    async enhanceAppointments(appointments) {
      ...
      for (const appointment of appointments) {
        ...
        const patientName = await this.getPatientDetails(patientEmail);
        enhancedAppointments.push({ ...appointment, patientName });
      }
      return enhancedAppointments;
    },
    async getPatientDetails(patientEmail) {
      try {
        const response = await axios.get(`http://127.0.0.1:5001/patient/${patientEmail}`);
        console.log("Appointment:" , response.data)
        if ('name' in response.data) {
          const patient = response.data.name;
          return patient;
        } else {
          console.error('Error: Name property not found in the response data');
        }
      } catch (error) {
        console.error(`Error fetching dentist name for email ${patientEmail}:`, error);
